...
{
	//待办事项：执行这个函数并测试射线是否与三角形(特殊点v0、v1、v2)相交
	//原点：orig 和 方向：dir
	//并且，不要忘记更新tnear,u和v
	Vector3f e1 = v1 - v0;
	Vector3f e2 = v2 - v0;
	...
	float sle1 = dotProduct(s1, e1);
	tnear = dotProduct(s2, e2) / sle1;
	u = dotProduct(s1, s) / sle1;
	v = dotProduct(s2, dir) / sle1;
...
class MeshTriangle : public Object
{
	...
	public:
		std::unique_ptr<Vector3f[]> vertices;					//顶点位置
		uint32_t numTriangles;									//三角形数目
